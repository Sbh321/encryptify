...
const handleGenerateMessage = async () => {
  // try {
  //   // URL for generating a random sentence
  //   const response = await fetch(
  //     "https://lorem-faker.vercel.app/api?paragraphs=1&format=text"
  //   );
  //   const data = await response.text();
  //   // Assuming the API returns plain text directly
  //   messageToEncrypt.value = data.trim(); // Use .trim() to remove any extra whitespace/newlines
  // } catch (error) {
...
<template>
  ...
  <div class="container">
    ...
    <button
      @click="handleEncrypt"
      class="bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600 transition duration-200 mb-2"
    >
      Encrypt Message
    </button>
